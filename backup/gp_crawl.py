...
file_handler = logging.FileHandler('gp_crawl.log')
...
def get_busy_hours(soup):
	...
	for i in range(len(days)):
		...
		for i in range(len(busyhours)):
			hourinfo=parse_busy_str(busyhours[i].attrs['aria-label'])
	...
	for i in range(7):
		...
		if len(week_busy_hours[i]) == 1:
			...
			logger.warning("The busyhour information for current station is in the different formation")
			raise
